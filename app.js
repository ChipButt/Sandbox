...
const ASTRA_ASSET_VERSION = '20260521-discovery-sequence-v1';
...
function typeTextToElement(el, fullText, speed=18){
...
function fadeInElement(el, duration=750){
  return new Promise(resolve=>{
    if(!el){ resolve(); return; }
    el.style.opacity='0';
...
function whenStageReady(stage, callback){
  if(!stage){ return; }
  const fire = ()=>requestAnimationFrame(()=>callback(stage));
  ...
  setTimeout(()=>{ try{ mo.disconnect(); }catch{} if(stage.isConnected) fire(); }, 2600);
}
async function runScannerDiscoveryReveal(stage, selectedId){
  if(!stage || !stage.isConnected || currentPage !== 'scanner') return;
  ...
  await delay(80);
  if(!stage.isConnected || currentPage !== 'scanner' || selectedAnomalyId !== selectedId) return;
  await typeTextToElement(nameEl, fullName, 24);
  if(!stage.isConnected || currentPage !== 'scanner' || selectedAnomalyId !== selectedId) return;
  await delay(110);
  await fadeInElement(anomalyImg, 800);
  if(!stage.isConnected || currentPage !== 'scanner' || selectedAnomalyId !== selectedId) return;
  await delay(90);
  await typeTextToElement(info1El, fullInfo1, 16);
  if(!stage.isConnected || currentPage !== 'scanner' || selectedAnomalyId !== selectedId) return;
  await delay(70);
  await typeTextToElement(info2El, fullInfo2, 16);
  if(!stage.isConnected || currentPage !== 'scanner' || selectedAnomalyId !== selectedId) return;
  await delay(70);
  await typeTextToElement(info3El, fullInfo3, 16);
}
...
function renderPage(pageName, selectedId=null, opts={}){
  ...
  if(pageName === 'scanner' && opts.discoveryReveal && selectedId){
    whenStageReady(stage, () => runScannerDiscoveryReveal(stage, selectedId));
  }
}
...
function handleDetectedScan(marker){
  ...
  setTimeout(()=>{
    ...
    renderPage('scanner',next.id,{discoveryReveal:true});
  },1200);
}
