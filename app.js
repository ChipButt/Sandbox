...
function scaleStage(){
  ...
  const viewportWidth = Math.max(1, Math.floor(vv ? vv.width : window.innerWidth));
  const viewportHeight = Math.max(1, Math.floor(vv ? vv.height : window.innerHeight));
...
function boot(){
  ...
  window.addEventListener('orientationchange', () => setTimeout(scaleStage, 80), {passive:true});
  if(window.visualViewport){
    window.visualViewport.addEventListener('resize', scaleStage, {passive:true});
    window.visualViewport.addEventListener('scroll', scaleStage, {passive:true});
  }
}
...
function createLayer(layer, pageName, selectedId, opts){
  ...
  if(layer.type === 'image'){
    ...
    if(pageName === 'home' && (layer.name || '') === 'inbox vertical transparent' && hasUnreadMessages()) el.classList.add('home-icon-attention-pulse');
...
function fontSizeForLayer(layer,pageName){
  if(pageName === 'dataLog' && String(layer.name||'').startsWith('Anomaly')) return 21;
...
function addHomeNodesToTracker(trackerEl){
  ...
  game.anomalies.forEach((a,i)=>{
    ...
    d.style.zIndex=6;
    const img=document.createElement('img');
    img.src=p.scanned[a.id] ? a.asset : 'assets/unknown-anomaly.png';
