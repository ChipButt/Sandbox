...
let lockedAppHeight = Math.max(window.innerHeight || 0, window.screen?.height || 0);
...
function scaleStage(){
  // Keep the app visually portrait and stable. Do not shrink the whole UI when the mobile keyboard appears.
  const keyboardOpen = isTextInputFocused();
  const viewportWidth = Math.max(1, window.innerWidth || document.documentElement.clientWidth || layout.stage.width);
  const rawHeight = Math.max(1, window.innerHeight || document.documentElement.clientHeight || layout.stage.height);
  if(!keyboardOpen){
    lockedAppHeight = Math.max(lockedAppHeight || 0, rawHeight);
  ...
  if(app){
    ...
    app.style.height = `var(--locked-app-height, ${viewportHeight}px)`;
    app.style.left = '0px';
    app.style.top = '0px';
  }
...
function createLayer(layer, pageName, selectedId, opts){
  ...
  if(layer.type === 'image'){
    ...
    if(pageName === 'scanner' && (layer.name || '').includes('unknown_anomaly')) {
      setAnomalyGlow(el, getAnomaly(selectedId));
      el.classList.add('anomaly-detail-entity');
    }
    ...
    if(pageName === 'dataLog' && (layer.name || '').includes('unknown_anomaly')) {
      ...
      el.classList.add('data-log-entity-pulse');
    }
...
function addImage(stage,src,x,y,w,h,z){ const d=document.createElement('div'); d.className='layer image'; Object.assign(d.style,{left:x+'px',top:y+'px',width:w+'px',height:h+'px',zIndex:z}); const img=document.createElement('img'); img.src=src; d.appendChild(img); if(String(src).includes('rb-logo')){ d.classList.add('clickable'); d.addEventListener('click',()=>window.open(ROLL_BRITANNIA_URL,'_blank','noopener')); } stage.appendChild(d); return d; }
