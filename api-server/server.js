...
async function createMockSheet(sessionDir) {
  const width = 1024;
  const height = 1024;
  const colours = {
    outline: "#2b1a12",
...
<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
  ...
  await saveBuffer(path.join(sessionDir, "nesw-source-sheet.png"), sheetBuffer);
  return sheetBuffer;
}
...
async function createAISheet(sessionDir, form, imageBuffer, imageMimeType, tweakNotes = "") {
  if (!openai) return createMockSheet(sessionDir);
    ...
    input: [
      {
        ...
        content: [
          ...
          { type: "input_image", image_url: `data:${imageMimeType};base64,${base64Ref}` }
        ]
      }
    ],
    tools: [{ type: "image_generation", action: process.env.IMAGE_TOOL_ACTION || "auto" }]
  });

  const imageBase64 = extractImageBase64FromResponse(response);

  if (!imageBase64) {
    await saveBuffer(path.join(sessionDir, "openai-response-debug.json"), Buffer.from(JSON.stringify(response, null, 2)));
    throw new Error("OpenAI did not return an image. A debug response was saved in the generated session folder.");
  }

  const buffer = Buffer.from(imageBase64, "base64");
  await saveBuffer(path.join(sessionDir, "nesw-source-sheet.png"), buffer);
  return buffer;
}
...
async function splitSheetIntoDirections(sessionDir, sheetBuffer, frameWidth, frameHeight) {
  ...
  for (const [dir, pos] of Object.entries(dirs)) {
    const buffer = await sharp(sheetBuffer)
      .extract({ left: pos.left, top: pos.top, width: quadWidth, height: quadHeight })
...
async function readSession(sessionId) {
  ...
  return { sessionDir, data: JSON.parse(raw) };
}
...
function buildCharacterJson(sessionData) {
  ...
  return {
    ...
    animation: {
      idle: { bobAmount: Number(form.idleBob || 1), bobSpeed: 900 },
      walk: { bobAmount: Number(form.walkBob || 2), bobSpeed: 180, swayAmount: Number(form.swayAmount || 1) },
      talk: { bobAmount: 1, bobSpeed: 220 }
    },
    npc: {
...
// Optional helper code for simple static-image animation.
...
Generated by Character Pack Builder.
...
The runtime helper provides simple code-driven animation from four static NESW images.
`;
}
...
app.get("/api/health", (req, res) => {
  res.json({
    ...
    message: openai ? "OpenAI API key detected." : "No OpenAI API key detected. Mock mode enabled."
  });
});
...
app.post("/api/generate", upload.single("reference"), async (req, res) => {
  try {
    ...
    let sourceBuffer = req.file?.buffer || Buffer.from("");
    let sourceMime = req.file?.mimetype || "image/png";

    if (req.file?.buffer) {
      const ext = path.extname(req.file.originalname || ".png") || ".png";
      await saveBuffer(path.join(sessionDir, `reference-original${ext}`), req.file.buffer);
    }
    ...
    await buildPreviewSheet(sessionDir, directionFiles, width, height);
    ...
    console.error(error);
    res.status(500).json({ ok: false, error: error.message || "Generation failed." });
  }
});
...
app.post("/api/export", async (req, res) => {
  try {
    ...
    await saveBuffer(path.join(sessionDir, `${slug}.json`), Buffer.from(JSON.stringify(buildCharacterJson(data), null, 2)));
    await saveBuffer(path.join(sessionDir, `${slug}.character.js`), Buffer.from(buildCharacterJs(data)));
    await saveBuffer(path.join(sessionDir, "animatedCharacterRuntime.js"), Buffer.from(buildRuntimeJs()));
    await saveBuffer(path.join(sessionDir, "README.txt"), Buffer.from(buildReadme(data)));
    ...
    res.json({ ok: true, downloadUrl: `/generated/${sessionId}/${packName}` });
  } catch (error) {
    console.error(error);
...
app.listen(PORT, () => {
  console.log(`Character Pack Builder API running at http://localhost:${PORT}`);
  console.log(openai ? "Mode: OpenAI image generation" : "Mode: mock generation.");
});