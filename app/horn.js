...
function registerHornBurst(durationMs) {
  ...
  setCurrentHornState({ ...hornState, bursts });
...
export async function onHornPress() {
  ...
  const hornState = getCurrentHornState();
  if (hornState.broken) {
    showStatus("Alert: Horn broken.", "error", 2200);
    showHornBrokenAlert();
    return;
  }
  if (isPlaying) return;
  ...
  try {
    ...
    const totalDuration = registerHornBurst(durationMs);
    breakAfterCurrentPlay = totalDuration >= HORN_THRESHOLD_MS;
    window.setTimeout(() => {
      isPlaying = false;
