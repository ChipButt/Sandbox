...
const INSTALL_HINT_KEY = `${APP_CONFIG.localSaveKey}_install_hint_seen`;
...
function boot() {
  ...
  renderInstallHint(shouldShowInstallHint());
  queueSync("boot");
}
...
function bindEvents() {
  ...
  els.dismissInstallHintBtn?.addEventListener("click", () => {
    localStorage.setItem(INSTALL_HINT_KEY, "1");
    renderInstallHint(false);
  });

  els.testNextRuneBtn?.addEventListener("click", handleTestNextRune);
  els.testLegendaryClaimBtn?.addEventListener("click", () => {
    handleRuneAbsorption({ type: "claim", id: "legendary", claimCode: APP_CONFIG.claimCodes.legendary, rawValue: "dev-test" });
  });
  els.testGodlikeClaimBtn?.addEventListener("click", () => {
    handleRuneAbsorption({ type: "claim", id: "godlike", claimCode: APP_CONFIG.claimCodes.godlike, rawValue: "dev-test" });
  });
...
function watchOnlineStatus() {
  ...
  window.addEventListener("offline", () => {
    renderConnectionStatus();
    renderSyncStatus(state, syncing);
    showMessage("warn", "Signal dropped. Your quest is still saved in this browser and will sync when connection returns.");
  });
}
...
async function openScanner() {
  ...
  const started = await startRuneScanner({
    ...
    readerEl: els.html5Reader,
    onStatus: setScannerStatus,
    onScan: scanned => {
      scannerActive = false;
      stopRuneScanner({ videoEl: els.scannerVideo, readerEl: els.html5Reader });
      hideScanner();
...
function handleTestNextRune() {
  const nextLocation = APP_CONFIG.runeLocations.find(location => !state.scannedRuneIds.includes(location.id));

  if (!nextLocation) {
    showMessage("good", "All six rune locations have already been absorbed. Godlike form reached.");
    return;
  }

  handleRuneAbsorption({ type: "scan", id: nextLocation.id, rawValue: "dev-test" });
}
...
function handleReset() {
  const confirmed = window.confirm("Reset the quest save in this browser? This is only for testing.");
  if (!confirmed) return;
  ...
  renderApp(state);
  renderSyncStatus(state, syncing);
  renderInstallHint(true);
  showMessage("warn", "This browser save has been reset for testing. Open the rune absorption window to begin again.");
}
