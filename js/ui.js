...
const els = {
  ...
  dismissInstallHintBtn: document.getElementById("dismissInstallHintBtn"),
  ...
  testNextRuneBtn: document.getElementById("testNextRuneBtn"),
  testLegendaryClaimBtn: document.getElementById("testLegendaryClaimBtn"),
  testGodlikeClaimBtn: document.getElementById("testGodlikeClaimBtn"),
  scannerOverlay: document.getElementById("scannerOverlay"),
  scannerVideo: document.getElementById("scannerVideo"),
  html5Reader: document.getElementById("html5Reader"),
...
export function renderInstallHint(show) {
  if (!els.installHint) return;
  els.installHint.classList.toggle("hidden", !show);
}
...
export function renderSyncStatus(state, syncing = false) {
  ...
  els.syncStatus.textContent = "Quest saved in this browser";
...
function renderSword(state) {
  ...
  els.swordDescription.textContent = state.claimed
    ...
    : stage.description;
  els.swordArt.innerHTML = createSwordMarkup(stage);
}
...
function renderDevPanel() {
  ...
  const shouldShow = APP_CONFIG.testToolsEnabled || params.get("dev") === "1";
...
function createSwordMarkup(stage) {
  if (stage.level === 0) {
    return `
  ...
  if (stage.asset) {
    return `<img class="sword-img" src="${escapeHtml(stage.asset)}" alt="${escapeHtml(stage.name)}" />`;
  }
