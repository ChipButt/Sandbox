...
export const ROUTE_VENUES = [
  {
    ...
    coords: { lat: 52.202913, lng: -1.796571 },
    socials: {
  ...
  },
  {
    ...
    coords: { lat: 52.155941, lng: -1.503302 },
    socials: {
  ...
  },
  {
    ...
    coords: { lat: 52.029466, lng: -1.75462 },
    socials: {
  ...
  },
  {
    ...
    coords: { lat: 51.929877, lng: -1.115896 },
    socials: {
  ...
  },
  {
    ...
    coords: { lat: 51.861614, lng: -2.221328 },
    socials: {
  ...
  },
  {
    ...
    coords: { lat: 52.277891, lng: -1.373323 },
    socials: {
  ...
  },
  {
    ...
    coords: { lat: 52.27724, lng: -1.38966 },
    socials: {
  ...
  },
  {
    ...
    coords: { lat: 51.974013, lng: -1.634296 },
    socials: {
...
export const GEOLOCATION_CACHE_KEY = "rrUserGeoV1";

export function calculateMilesBetween(pointA, pointB) {
...
export function formatDistanceMiles(distanceMiles) {
  if (!Number.isFinite(distanceMiles)) return "Distance unavailable";
  if (distanceMiles < 10) return `${distanceMiles.toFixed(1)} miles away`;
  return `${Math.round(distanceMiles)} miles away`;
}

export function getVenueDistanceMiles(venue, userCoords) {
  if (!venue?.coords || !userCoords) return null;
  return calculateMilesBetween(userCoords, venue.coords);
}

function readCachedUserCoords() {
  try {
    const raw = localStorage.getItem(GEOLOCATION_CACHE_KEY);
    if (!raw) return null;
...
function cacheUserCoords(coords) {
  try {
    localStorage.setItem(GEOLOCATION_CACHE_KEY, JSON.stringify({ ...coords, timestamp: Date.now() }));
  } catch {}
}
...
export function getVenueOpenState(venue, date = new Date()) {
  const { day, minutes } = getUkNowParts(date);
  const raw = String(venue?.opening?.[day] || "Closed").trim();

  if (!raw || /^closed$/i.test(raw)) {
    return { label: "Closed now", isOpen: false, detail: raw || "Closed" };
  }

  if (!/\d/.test(raw)) {
    return { label: "Check hours", isOpen: false, detail: raw };
  }

  const intervals = raw
    .split(/\s*,\s*/)
    .map((entry) => entry.match(/(\d{1,2}:\d{2}\s*[AP]M)\s*[–-]\s*(\d{1,2}:\d{2}\s*[AP]M)/i))
    .filter(Boolean)
    .map((match) => [parseClockValue(match[1]), parseClockValue(match[2])])
  ...
  const isOpen = intervals.some(([start, end]) => minutes >= start && minutes <= end);
  return {
